...
#endif
{
   ...
   for(int i=1;i<argc;i++)
   {
      ...
      if(STRCMP("--my_index",argv[i])==0)
      {
         ...
         ++i;
         #ifdef __WX__CRYST__
         wxString tmp_argv1(argv[i]);
         ifstream f1(tmp_argv1.ToAscii());
         #else
         ifstream f1(argv[i]);
         #endif
         if(!f1)
         {
            cout<<"Cannot find integrals file:"<<argv[i]<<endl;
            exit(0);
         }
         pl.ImportIntegrals(f1);
         f1.close();

         // CellExplorer cx(pl,MONOCLINIC,4);
         CellExplorer cx(pl,MONOCLINIC,4);
         cx.SetAngleMinMax((float)90*DEG2RAD,(float)120*DEG2RAD);
         ...
         const float vmax=EstimateCellVolume(dmin,dmax,nb,TRICLINIC  ,LATTICE_P,0.2);
         // // debug
         // // float vmax = 3*EstimateCellVolume(dmin,dmax,nb,TRICLINIC  ,LATTICE_P,0.2);
         // float vmax = 15625;
      ...
      if(STRCMP("--index-test",argv[i])==0)
      {
         ...
         for(unsigned int k=0;k<100;++k)
         {
            ...
            const std::list< std::pair< RecUnitCell, double > >::const_iterator pos=cx.GetSolutions().begin();
            double score=0,vsol=0;
