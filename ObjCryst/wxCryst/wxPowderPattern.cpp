...
void WXPowderPatternGraph::OnLoadPeaks(wxCommandEvent& WXUNUSED(event))
{
   ...
   // ifstream f3(fn.GetPath().ToAscii());
   // assert(f3);
   // f3.close();
   ...
   wxString peaks_filename = fn.GetPath().ToAscii();
   cout << peaks_filename << endl;
   // ifstream f1(peaks_filename);
   // ifstream f1(fn.GetPath().ToAscii());
   // assert(f1);
   // char tmp[512];
   // f1.read(tmp, 10);
   // cout << tmp << endl;
   ...
   double min_tt = mX.data()[0];
   double max_tt = mX.data()[mX.size() - 1];

   double lam = mpPattern->GetPowderPattern().GetWavelength();
   cout << peaks_filename << " " << min_tt << " " << max_tt << " " << lam << endl;
   // assert(0);

   // cout << __LINE__ << endl;
   // cout << mX.size() << " " << mX.data()[0] << " " << mX.data()[mX.size() - 1]  << endl;
   // // m2theta.data()[0] << " " << m2theta[m2theta.size() - 1] 
   // assert(0);

   // sprintf(temp, "./make_all %s %f %f %f", peaks_filename, min_tt, max_tt, lam);

   char temp[512];
   sprintf(temp, " %f %f %f", min_tt, max_tt, lam);
   for (int i = 0; i != 512; ++i)
   {
      if (temp[i] == ',') 
      {
   ...
   wxString temp_wx(temp);
   wxString command_name("./FOX_data/process_peaks ");
   wxString command = command_name + peaks_filename + temp_wx;
   ...
   // replace_if(temp_string.begin(), temp_string.end(), IsComma, '.');
   ...
   ifstream f("FOX_data/exp_intervals.txt");
   if(!f) return;//:TODO:
   ...
   // mine
   mPeakList.ImportDhklDSigma(f);
   f.close();

   ifstream f2("FOX_data/integrals.txt");
   f2.imbue(std::locale::classic());
   mPeakList.ImportIntegrals(f2);
   f2.close();
...
wxWindow(parent,-1),mpGraph(graph),mpPeakList(&peaklist),mpCellExplorer(0),mpCrystal(0),mpDiff(0)
{
   ...
   // Advanced interface
      ...
      bravaisChoices.Add(_T("Rhombohedral"));
...
void WXCellExplorer::OnIndex(wxCommandEvent &event)
{
   ...
   if(event.GetId()==ID_CELLEXPLORER_INDEX_QUICK)
   {
      ...
      while(nbSpurious<=maxNbSpurious)
      {
         ...
         if((mpCellExplorer->GetBestScore()<=stopOnScore)||continueOnSolution)
         {
            minv=EstimateCellVolume(dmin,dmax,nb,RHOMBOHEDRAL,LATTICE_P,1.5);
            maxv=EstimateCellVolume(dmin,dmax,nb,RHOMBOHEDRAL,LATTICE_P,0.4*weak_f);
            mpCellExplorer->SetVolumeMinMax(minv,maxv);
            lengthmax=pow(maxv,(float)(1/3.0))*3;
            if(lengthmax<25)lengthmax=25;
            mpCellExplorer->SetLengthMinMax(3,lengthmax);
            mpCellExplorer->SetCrystalSystem(RHOMBOHEDRAL);
            mpCellExplorer->SetCrystalCentering(LATTICE_P);
            mpLog->AppendText(wxString::Format(_T("RHOMBOHEDRAL  : V= %6.0f -> %6.0f A^3, max length=%6.2fA"),minv,maxv,lengthmax));
            t0=chrono.seconds();
            if(dlgProgress.Update(2,wxString::Format(_T("RHOMBOHEDRAL (%u spurious), V=%6.0f-%6.0f, l<%6.2fA\n")
                                                   _T("Best Score=%6.1f"),
                                                   nbSpurious,minv,maxv,lengthmax,mpCellExplorer->GetBestScore()))==false) break;
            mpCellExplorer->DicVol(reportOnScore,reportOnDepth,stopOnScore,stopOnDepth);
            mpLog->AppendText(wxString::Format(_T(" -> %3u sols in %6.2fs, best score=%6.1f\n"),
   ...
   if(mpCellExplorer->GetSolutions().size()>0)
   {
      ...
      wxArrayString sols;
      float bestvol=0;
      for(list<pair<RecUnitCell,double> >::const_iterator pos=mpCellExplorer->GetSolutions().begin();
         pos!=mpCellExplorer->GetSolutions().end();++pos)
      {
         ...
         switch(pos->first.mlattice)
         {
            ...
            case RHOMBOHEDRAL:sys="RHOMBOHEDRAL"; break;
...
void WXCellExplorer::OnSelectCell(wxCommandEvent &event)
{
   ...
   if(choice!=wxNOT_FOUND)
   {
      ...
      list<pair<RecUnitCell,double> >::const_iterator pos=mpCellExplorer->GetSolutions().begin();
         ...
         switch(pos->first.mCentering)
         {
            case LATTICE_P:
            {
               switch(pos->first.mlattice)
               {
                  ...
                  case RHOMBOHEDRAL:mpCrystal->GetSpaceGroup().ChangeSpaceGroup("R-3m");break;
