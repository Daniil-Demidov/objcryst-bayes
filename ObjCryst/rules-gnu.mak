...
# my cases
# debug with optimizations - fast enough
 else ifeq ($(debug),2)
   ifdef RPM_OPT_FLAGS
      # we are building a RPM !
      CPPFLAGS = ${RPM_OPT_FLAGS}
      ...
      CPPFLAGS = -O3 -w -ffast-math -fstrict-aliasing -pipe -fno-omit-frame-pointer -funroll-loops -ftree-vectorize -pg ${SSE_FLAGS} ${COD_FLAGS}
   endif
   DEPENDFLAGS = ${SEARCHDIRS} ${GL_FLAGS} ${WXCRYSTFLAGS} ${FFTW_FLAGS} ${REAL_FLAG}
   LOADLIBES = -lm -lcryst -lCrystVector -lQuirks -lRefinableObj -lcctbx ${LDNEWMAT} ${PROFILELIB} ${GL_LIB} ${WX_LDFLAGS} ${FFTW_LIB}
# no debug no optimization - minimum flags
 else ifeq ($(debug),3)
 #Set DEBUG options
   ifdef RPM_OPT_FLAGS
      # we are building a RPM !
   ...
   ifdef RPM_OPT_FLAGS
      ...
      #default flags - use "sse=1" to enable SSE optimizations
      # my variant - with warnings
      CPPFLAGS = -O3 -Wall -ffast-math -fstrict-aliasing -pipe -fomit-frame-pointer -funroll-loops -ftree-vectorize ${SSE_FLAGS} ${COD_FLAGS}
      # old variant - inhibit warnings
